          ...
          <div className="grid gap-2" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))' }}>
            {products?.map(p => (
              <div key={p.id} className="bg-stokiloo-dim border border-stokiloo-border overflow-hidden group hover:border-stokiloo-gold/40 transition-colors">
                ...
                <div className="p-1">
                  <div className="font-bold text-[9px] text-stokiloo-white truncate leading-tight" title={p.name}>{p.name}</div>
                  <div className="flex items-center justify-between mt-0.5">
                    <span className="font-bold text-[9px] text-stokiloo-gold font-mono">{new Intl.NumberFormat('fr-DZ', { style: 'currency', currency: 'DZD' }).format(p.price)}</span>
                    <span className={`text-[7px] font-bold ${p.stock_quantity > 0 ? 'text-stokiloo-emerald' : 'text-stokiloo-rose'}`}>{p.stock_quantity}</span>
                  </div>
                </div>
              </div>
            ))}
